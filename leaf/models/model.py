...
class Model(ABC):
    ...
    def __init__(self, seed, lr, optimizer=None):
        ...
        with self.graph.as_default():
            ...
            self.features, self.labels, self.train_op, self.eval_metric_ops, self.loss, self.logits, self.classes = self.create_model()
    ...
    def test(self, data):
        ...
        with self.graph.as_default():
            labels, predictions, tot_acc, loss, logits = self.sess.run(
                [self.labels,self.classes, self.eval_metric_ops, self.loss, self.logits],
                feed_dict={self.features: x_vecs, self.labels: labels}
            )
        acc = float(tot_acc) / x_vecs.shape[0]
        out = {'features':x_vecs ,ACCURACY_KEY: acc, 'loss': loss, 'ground_thruth': labels, 'prediction':predictions}
        for i in range(len(logits[0])):
            out['c'+str(i)] = []
            for j in range(len(logits)):
                out['c'+str(i)].append(logits[j][i])
        return out
